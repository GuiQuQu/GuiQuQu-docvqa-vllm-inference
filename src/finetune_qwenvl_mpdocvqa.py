...
class ModelArguments:
    model_name_or_path: Optional[str] = field(default="/root/pretrain-model/Qwen-VL-Chat-Int4")
    ...
    for i, msg in enumerate(messages):
        ...
        for j, sentence in enumerate(msg):
            def get_system_message(system_message):
                system_input_id = im_start + _system + tokenizer(system_message).input_ids + im_end + nl_tokens
                system_target_id = im_start + [IGNORE_TOKEN_ID] * (len(system_input_id)-3) + im_end + nl_tokens
